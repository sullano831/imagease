...
const THUMB_MAX = 260 // max thumbnail display width in px
...
function ImageCard({ result, format, darkMode }) {
  ...
  return (
    <div className={`image-card ${darkMode ? 'dark' : ''}`}>
      <div className="thumb-wrapper" style={{ width: tw, height: th }}>
        <img src={result.dataURL} alt={result.name} style={{ width: tw, height: th }} />
      </div>
      <div className="card-info">
        <span className="card-name">{result.name}</span>
        <span className="card-dims">{result.width} × {result.height}</span>
      </div>
      <button
...
export default function App() {
  ...
  const [sourceImage, setSourceImage] = useState(null) // { file, img, src }
  const [results, setResults] = useState([]) // { name, width, height, dataURL }
  ...
  const processImage = useCallback(async (img, allSizes) => {
    setProcessing(true)
    const mime = MIME_MAP[format]
    const processed = allSizes.map(size => ({
      name: size.name || `custom-${size.width}x${size.height}`,
      width: size.width,
      height: size.height,
  ...
  const handleFile = useCallback(async (file) => {
    ...
    const validCustom = customSizes.filter(s => s.width && s.height)
    const allSizes = [...PRESET_SIZES, ...validCustom.map(s => ({
      name: s.name || `custom-${s.width}x${s.height}`,
      width: Number(s.width),
      height: Number(s.height),
    }))]
    await processImage(img, allSizes)
  }, [customSizes, processImage])
  ...
  const reprocess = async () => {
    ...
    await processImage(sourceImage.img, allSizes)
  }

  // Re-process when format changes and we already have an image
  const handleFormatChange = async (newFmt) => {
    setFormat(newFmt)
    if (!sourceImage) return
    setProcessing(true)
    const mime = MIME_MAP[newFmt]
    const validCustom = customSizes.filter(s => s.width && s.height)
    const allSizes = [...PRESET_SIZES, ...validCustom.map(s => ({
      name: s.name || `custom-${s.width}x${s.height}`,
      width: Number(s.width),
    ...
    const processed = allSizes.map(size => ({
      ...
      width: size.width,
      height: size.height,
      dataURL: cropCover(sourceImage.img, size.width, size.height, mime),
    }))
    setResults(processed)
    setProcessing(false)
  }
  ...
  const downloadAll = async () => {
    ...
    results.forEach(r => {
      ...
      zip.file(`${r.name}.${ext}`, blob)
    })
    const content = await zip.generateAsync({ type: 'blob' })
    const url = URL.createObjectURL(content)
    const a = document.createElement('a')
    a.href = url
    a.download = `images-${format}.zip`
    a.click()
    URL.revokeObjectURL(url)
  }

  const addCustomSize = () => {
    setCustomSizes(prev => [...prev, { id: Date.now(), name: '', width: '', height: '' }])
  }
  ...
  const updateCustomSize = (id, field, value) => {
    setCustomSizes(prev => prev.map(s => s.id === id ? { ...s, [field]: value } : s))
  }
      ...
      <main className="main">
        ...
        {!sourceImage && (
          <section className="hero-section">
            <h1 className="hero-title">Resize & Crop Images Instantly</h1>
            <p className="hero-sub">Upload once. Get perfectly cropped versions for every slot — no distortion, centered cover crop.</p>
            ...
            >
              ...
              />
              <UploadIcon />
              <p className="upload-label">Drop an image here or <span className="upload-link">browse</span></p>
            ...
            {/* Customize Size section */}
            <div className="customize-section">
              <button
                className={`btn btn-ghost ${showCustom ? 'active' : ''}`}
        ...
        {sourceImage && (
          <section className="results-section">
            ...
            {/* Custom sizes (shown in results too) */}
            <div className="customize-section">
              <button
                className={`btn btn-ghost ${showCustom ? 'active' : ''}`}
                onClick={() => setShowCustom(v => !v)}
            ...
            {processing ? (
              ...
              <div className="results-grid">
                {results.map(r => (
                  <ImageCard key={r.name} result={r} format={format} darkMode={darkMode} />
                ))}
              </div>
            )}
          </section>
